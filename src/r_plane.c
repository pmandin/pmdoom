...
//-----------------------------------------------------------------------------
//
// $Id: r_plane.c,v 1.1 2006/08/18 19:01:29 patrice Exp $
//
...
static unsigned short *visplanesy;

// ?
#define MAXOPENINGS	(sysvideo.width*64)
static short			*openings=NULL;
short*			lastopening;
  ...
  int		lightlevel )
{
	...
	memset (check->top,0xff,sysvideo.width*sizeof(unsigned short));
	memset (check->bottom, 0, sysvideo.width*sizeof(unsigned short));
  ...
  int		stop )
{
	...
	memset (pl->top,0xff,sysvideo.width*sizeof(unsigned short));
	memset (pl->bottom, 0, sysvideo.width*sizeof(unsigned short));
...
void R_DrawPlanes (void)
{
	...
	for (pl = visplanes ; pl < lastvisplane ; pl++) {
		...
		if (pl->picnum == skyflatnum) {
			dc_iscale = (FRACUNIT*SCREENHEIGHT)/viewheight;
