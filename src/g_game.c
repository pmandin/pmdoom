...
boolean		mousearray[NUM_MOUSE_BUTTONS]; 
...
boolean         joyarray[NUM_JOY_BUTTONS]; 
...
{ 
	...
	if (strafe) { 
		...
		if (joyxmove != 0) 
			side += (sidemove[speed] * joyxmove)/128; 
    ...
    if (joyymove != 0) 
		forward -= (forwardmove[speed] * joyymove)/128; 
		...
		case ev_mouse: 
			{
				...
				for (i=0;i<NUM_MOUSE_BUTTONS-1; i++) {
					mousebuttons[i] = ev->data1 & (1<<i); 
				}
		...
		case ev_joystick: 
			{
				...
				for (i=0;i<NUM_JOY_BUTTONS-1; i++) {
					joybuttons[i] = ev->data1 & (1<<i); 
				}
				joyxmove = ev->data2; 
				joyymove = ev->data3; 
				return true;    // eat events 
...
{ 
	...
	sprintf(name,"c:\\doomdata\\"SAVEGAMENAME"%d.dsg",savegameslot);
		...
		if (!home) {
			...
			sprintf (name, "%s/.doom/"SAVEGAMENAME"%d.dsg",home,savegameslot); 
		}
	}
