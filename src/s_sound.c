...
//-----------------------------------------------------------------------------
//
// $Id: s_sound.c,v 1.1 2006/08/18 19:01:29 patrice Exp $
//
  ...
  int		volume )
{
	...
	//  mix/output buffer.
	vol1 = ((volume<<4)|volume)>>1;
	
	channels[cnum].handle = I_StartSound(sfx_id, vol1, sep, pitch,priority);
}	
...
/*
	...
	if (mus_playing && !I_QrySongPlaying(mus_playing->handle)
		&& !mus_paused )
	{
		S_StopMusic();
	}
