...
//-----------------------------------------------------------------------------
//
// $Id: w_wad.c,v 1.1 2006/08/18 19:01:29 patrice Exp $
//
...
lumpinfo_t*		lumpinfo;		
int			numlumps;
static int	lumpinfo_size;	/* for realloc */
...
void W_Reload (void)
{
	...
	f_info = fileinfo;
	for (i=reloadlump ; i<reloadlump+lumpcount ; i++,lump_p++, f_info++) {
		if (lumpcache[i])
			Z_Free (lumpcache[i]);
...
{	
	...
	lumpinfo = Z_Malloc(LUMPINFO_START_SIZE, PU_STATIC, NULL);	
	lumpinfo_size = LUMPINFO_START_SIZE;
